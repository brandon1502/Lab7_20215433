...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.formLogin()
                ...
                .successHandler((request, response, authentication) -> {
                    ...
                    if (defaultSavedRequest != null) {
                        ...
                        if (rol.equals("admin")) {
                            response.sendRedirect("/funciones");
                        } else if (rol.equals("gerente")) {
                            response.sendRedirect("/salas");
                        }else {
                            response.sendRedirect("/obrasDisponibles");
                        }
                    }
                });
        /*
        ...
        http.authorizeHttpRequests()
                .requestMatchers("/salas","/salas/**").hasAnyAuthority("gerente")
                .requestMatchers("/obras","/obras/**").hasAnyAuthority("gerente")
                .requestMatchers("/funciones","/funciones/**").hasAnyAuthority("gerente", "admin")
                .requestMatchers("/reservas","/reservas/**").hasAnyAuthority( "gerente","admin")
                .requestMatchers("/obrasDisponibles","/obrasDisponibles/**").hasAnyAuthority("cliente")
                .anyRequest().permitAll();

        http.logout()
                .logoutSuccessUrl("/peliculas")
    ...
    public UserDetailsManager users(DataSource dataSource) {
        ...
        String sqlAuth = "SELECT email,password,activo FROM users where email = ?";
